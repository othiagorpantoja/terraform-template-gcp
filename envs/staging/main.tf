...
module "network" {
  ...
  ssh_source_ranges    = ["0.0.0.0/0"] # Configure com IPs específicos
}
...
module "sql" {
  ...
  tier                             = "db-g1-small" # Instância intermediária para staging
...
module "storage" {
  ...
  environment               = var.environment
  location                  = "US"
  force_destroy             = true # Permite destruição em staging
  create_backup_bucket      = true
  create_logs_bucket        = true
  private_lifecycle_age     = 60
  public_lifecycle_age      = 15
  backup_lifecycle_age      = 90
  logs_lifecycle_age        = 15
}
...
module "vm" {
  ...
  preemptible             = false # Staging usa instâncias estáveis
